...
if (!empty($_GET['search'])) {
    $data = $_GET['search'];
    $sql = "SELECT * FROM medio WHERE idmed  LIKE '%$data%' or nome LIKE '%$data%' ORDER BY idmed  DESC";
...
<div class="container">
   ...
   <div class="content">
    ...
    <div class="container my-5">
        <h2>Lista de intituições de ensino medio</h2>
        <table class="table">
            <thead>
                <tr>
                    <th scope="col">ID</th>
            ...
            <td>{$row['idCurso']}</td>
            <td>
              <a class='btn btn-outline-primary btn-sm' href='editarmedeviews.php?id={$row['idmed']}'>
                <img  src='imagem/pencil.svg' alt='bootstrap'>
              </a>
            </td>
            <td>
              <a class='btn btn-outline-danger btn-sm' href='apagarmedeviews.php?id={$row['idmed']}'>
...
<script>
    ...
    function searchData() {
        window.location = 'medeviews.php?search=' + search.value;
    }
</script>
