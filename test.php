...
<body>
    ...
    $sql_medio = "SELECT * FROM medio WHERE medio.curso = 'informatica'";

    $result_medio = $mysqli->query($sql_medio);

    if ($result_medio->num_rows > 0) {
        echo "<h2>Os institutos encontrados foram</h2>";
        echo "<table><tr><th>ID</th><th>Nome</th><th>Endereço</th><th>Curso</th><th>Proprina</th></tr>";
        while ($row = $result_medio->fetch_assoc()) {
            echo "<tr><td>" . $row["idmed"] . "</td><td>" . $row["instituicao"] . "</td><td>" . $row["endereco"] . "</td><td>" . $row["curso"] . "</td><td>" . $row["proprina"] . "</td></tr>";
        }
        echo "</table>";
    } else {
    ...
    $sql_comedio = "SELECT * FROM comedio1 WHERE comedio1.curso = 'informatica'";
